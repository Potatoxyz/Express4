...
router.get('/filesList', function (req, res) {
    //读取本地文件
    var fileArray = fs.readdirSync(path.resolve(__dirname, '../public/upload'));
    var filelist = [];
...
router.post('/addfile', function (req, res) {
    ...
    var exist = fs.existsSync(path.resolve(__dirname, '../public/upload/' + filename));
    if (!exist) {
    ...
    try {
        for (var key in targetFileName) {
            ...
            arr.forEach(function (value) {
                console.log(value);
                fs.readdir(path.resolve(__dirname, '../public/upload/' + value), function (err, files) {
                    //是否是空文件夹,如果是，先删除子文件
                    if (files) {
...
//编辑文件夹
router.post('/editfile', function (req, res) {
    //console.log(req.body);
    var beforetext = req.body.beforetext;
    var aftertext = req.body.aftertext;
    //要修改的文件夹存在，修改之后不能同名
    var exist = fs.existsSync(path.resolve(__dirname, '../public/upload/' + beforetext));
    if (!exist) {
        res.send('文件不存在');
    }
    else {
...
var targetFile = '';
//上传图片之前先验证文件夹
router.post('/targetFile', function (req, res) {
    targetFile = req.body.targetFile;
    var targetfilePath = path.resolve(__dirname, '../public/upload/' + targetFile);
...
router.get('/getPic', function (req, res) {
    ...
    if (targetfile) {
        try {
            fs.readdir(path.resolve(__dirname, '../public/upload/' + targetfile), function (err, files) {
                if (files) {
                    var urls = [];
